...
function _buildCustomTemplates(platList) {
  const templates = [];
  let tmplId = 1;
  // templateId must be a small sequential integer (GTM int32 limit).
  // The fingerprint is what links tag type 'cvt_<fingerprint>' to the template.
  if (platList.includes('meta')   && TPL_META)
    templates.push({ accountId: '0', containerId: '0', templateId: String(tmplId++), name: 'ET - Meta CAPI',                       fingerprint: FP_META,   templateData: TPL_META });
  if (platList.includes('tiktok') && TPL_TIKTOK)
    templates.push({ accountId: '0', containerId: '0', templateId: String(tmplId++), name: 'ET - TikTok Events API',               fingerprint: FP_TIKTOK, templateData: TPL_TIKTOK });
  if (platList.includes('snap')   && TPL_SNAP)
    templates.push({ accountId: '0', containerId: '0', templateId: String(tmplId++), name: 'ET - Snapchat CAPI',                   fingerprint: FP_SNAP,   templateData: TPL_SNAP });
  if (platList.includes('gads')   && TPL_GADS)
    templates.push({ accountId: '0', containerId: '0', templateId: String(tmplId++), name: 'ET - Google Ads Enhanced Conversions', fingerprint: FP_GADS,   templateData: TPL_GADS });
  return templates;
}
