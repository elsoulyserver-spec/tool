...
// ─────────────────────────────────────────────────────────────────────────────
// Stable fingerprints for inline custom templates.
// GTM identifies a custom template by its fingerprint and wires it to tags via
// type: 'cvt_<fingerprint>'. These values are fixed so the JSON is deterministic
// across builds — never change them or existing imported containers will break.
// ─────────────────────────────────────────────────────────────────────────────
const FP_META   = '7381947261830';
const FP_TIKTOK = '8294716253841';
const FP_SNAP   = '5162839471820';
const FP_GADS   = '3847162938471';
...
function _buildCustomTemplates(platList) {
  ...
  if (platList.includes('meta')   && TPL_META)
    templates.push({ name: 'ET - Meta CAPI',                       fingerprint: FP_META,   templateData: TPL_META });
  if (platList.includes('tiktok') && TPL_TIKTOK)
    templates.push({ name: 'ET - TikTok Events API',               fingerprint: FP_TIKTOK, templateData: TPL_TIKTOK });
  if (platList.includes('snap')   && TPL_SNAP)
    templates.push({ name: 'ET - Snapchat CAPI',                   fingerprint: FP_SNAP,   templateData: TPL_SNAP });
  if (platList.includes('gads')   && TPL_GADS)
    templates.push({ name: 'ET - Google Ads Enhanced Conversions', fingerprint: FP_GADS,   templateData: TPL_GADS });
  return templates;
}
 ...
 *                Meta CAPI   — inline template, type cvt_7381947261830
 *                TikTok Events API — inline template, type cvt_8294716253841
 *                Snapchat CAPI     — inline template, type cvt_5162839471820
 *                Google Ads EC     — inline template, type cvt_3847162938471
 *
...
function buildServerConfig({
  ...
  // Google Ads EC — inline custom template (cvt_FP_GADS)
  if (platList.includes('gads') && px.gads && TPL_GADS) {
    evList.filter(k => GADS_EVENT[k]).forEach(key => {
      ...
      tags.push({
        ...
        firingTriggerId: [tid],
        tagFiringOption: 'oncePerEvent',
        notes: `EasyTrac — Google Ads EC ${GADS_EVENT[key]} | inline template cvt_${FP_GADS} | Generated by EasyTrac`,
      });
    });
  }
  ...
  // Each template carries a stable fingerprint. GTM wires it to tags that use
  // type: 'cvt_<fingerprint>' — the container JSON is fully self-contained.
  // Importing this JSON once into sGTM registers both templates AND tags together.
  const customTemplate = _buildCustomTemplates(platList);

  const containerVersion = { variable: variables, trigger: triggers, tag: tags, client };
  if (customTemplate.length > 0) containerVersion.customTemplate = customTemplate;

  return {
    exportFormatVersion: 2,
    containerVersion,
    _meta: {
      createdBy: 'EasyTrac GTM Config Builder v3.2',
      architecture: 'Web GTM to GA4 (transport_url) to Server GTM (GA4 Client) to Platform APIs',
      capiMode: 'inline-custom-templates-cvt-fingerprint',
      officialTemplates: false,
      inlineTemplates: customTemplate.map(t => t.name),
