...
// ID must fit in a 32-bit signed integer (max 2147483647).
// ─────────────────────────────────────────────────────────────────────────────
const FP_HTTP = '1780799001';
...
function buildServerConfig({
  ...
  // Meta CAPI — universal HTTP forwarder (cvt_1780799001)
    ...
    evList.forEach(key => {
      ...
      tags.push({
        ...
        parameter: [
          ...
          { type: 'TEMPLATE', key: 'clientIp',   value: '{{ET - client_ip_clean}}' },
          { type: 'TEMPLATE', key: 'userAgent',  value: '{{ET - Header user_agent}}' },
          { type: 'BOOLEAN',  key: 'enableDebug', value: 'false' },
        ],
        firingTriggerId: [tid],
        tagFiringOption: 'ONCE_PER_EVENT',
        notes: `EasyTrac — Meta CAPI ${mEv} | cvt_${FP_HTTP} universal forwarder`,
      });
    });
  }

  // TikTok Events API — universal HTTP forwarder (cvt_1780799001)
  if (platList.includes('tiktok') && px.tiktok && tok.tiktok) {
    evList.forEach(key => {
      ...
      tags.push({
        ...
        firingTriggerId: [tid],
        tagFiringOption: 'ONCE_PER_EVENT',
        notes: `EasyTrac — TikTok Events API ${ttEv} | cvt_${FP_HTTP} universal forwarder`,
      });
    });
  }

  // Snapchat CAPI — universal HTTP forwarder (cvt_1780799001)
  if (platList.includes('snap') && px.snap && tok.snap) {
    evList.forEach(key => {
      ...
      tags.push({
        ...
        firingTriggerId: [tid],
        tagFiringOption: 'ONCE_PER_EVENT',
        notes: `EasyTrac — Snapchat CAPI ${sEv} | cvt_${FP_HTTP} universal forwarder`,
      });
    });
  }

  // Google Ads Enhanced Conversions — native aw_ct tag (no custom template)
    ...
    evList.forEach(key => {
      ...
      tags.push({
        ...
        parameter: [
          ...
          { type: 'TEMPLATE', key: 'conversionValue', value: '{{ET - epn value}}' },
          { type: 'TEMPLATE', key: 'currencyCode',    value: '{{ET - ep currency}}' },
        ...
        firingTriggerId: [tid],
        tagFiringOption: 'ONCE_PER_EVENT',
        notes: `EasyTrac — Google Ads EC ${gEv} | native aw_ct tag`,
      });
    });
  }
