...
function buildServerConfig({
  ...
  // v3: the universal-http template reads all canonical fields (ep.* / up.*)
  // directly from getEventData(). Only HTTP-header-derived values must be passed
  // as template parameters because custom templates cannot access sGTM headers
  // natively — they must be injected from server-side header variables.
  const _capiUserParams = [
    { type: 'TEMPLATE', key: 'clientIp',  value: '{{ET - Header client_ip}}' },
