 ...
 * Each entry embeds the full .tpl source so the template is registered
 * automatically when the server container JSON is imported into sGTM.
 *
...
function _buildCustomTemplates(platList) {
  ...
  if (platList.includes('meta')   && TPL_META)
    templates.push({ name: 'ET - Meta CAPI (Manual HTTP)',                       templateData: TPL_META });
  if (platList.includes('tiktok') && TPL_TIKTOK)
    templates.push({ name: 'ET - TikTok Events API (Manual HTTP)',               templateData: TPL_TIKTOK });
  if (platList.includes('snap')   && TPL_SNAP)
    templates.push({ name: 'ET - Snapchat CAPI (Manual HTTP)',                   templateData: TPL_SNAP });
  if (platList.includes('gads')   && TPL_GADS)
    templates.push({ name: 'ET - Google Ads Enhanced Conversions (Manual HTTP)', templateData: TPL_GADS });
  return templates;
}
...
function buildServerConfig({
  ...
  // Embedding the .tpl sources here means the server container JSON is self-contained.
  // When imported into sGTM, GTM registers each template automatically so the
  // et_*_manual tag types resolve without a separate manual import step.
  ...
  return {
    ...
    containerVersion,
    _meta: {
      createdBy: 'EasyTrac GTM Config Builder v3.1',
      architecture: 'Web GTM → GA4 (transport_url) → Server GTM (GA4 Client) → Platform APIs',
      capiMode: 'manual-http-custom-templates-inline',
