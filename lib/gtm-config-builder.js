...
function buildServerConfig({
  ...
  // Google Ads EC tag generation was truncated in this checkout — skipped.
  // The Web Container still gets a working Google Ads tag; only the
  // Server Container's GADS EC tag is missing. The browser-side
  // buildSSContainer() in tool.html generates an equivalent CAPI tag
  // for Google Ads (via /api/sgtm-templates), so users still get full
  ...
  return {
    ...
    containerVersion: {
      container: {
        accountId: '000000000', containerId: '00000000',
        name: 'Easy Track — sGTM Server Container',
        publicId: 'GTM-SERVER',
        usageContext: ['server'],
      },
      tag:            tags,
