...
function buildServerConfig({
  ...
  // Computed: first IP from x-forwarded-for (strips proxy chain)
  variables.push(sjtVar('ET - client_ip_clean', `function() {
  var raw = {{ET - Header client_ip}};
  if (!raw) return '';
  ...
  // ── Computed Variables ─────────────────────────────────────────────────────
  variables.push(sjtVar('ET - event_time_unix', `function() {
  var ep = {{ET - ep event_time}};
  if (ep && !isNaN(Number(ep))) return Number(ep);
  return Math.floor(Date.now() / 1000);
}`));

  variables.push(sjtVar('ET - resolved_fbc', `function() {
  var fbc = {{ET - up fbc}} || {{ET - ep _fbc}} || '';
  if (fbc && fbc !== '') return fbc;
  var fbclid = {{ET - ep fbclid}} || '';
  if (!fbclid) return '';
  return 'fb.1.' + Math.floor(Date.now() / 1000) + '.' + fbclid;
}`));

  variables.push(sjtVar('ET - resolved_fbp', `function() {
  return {{ET - up fbp}} || {{ET - ep _fbp}} || '';
}`));
