...
function buildServerConfig({
  ...
  // ── Plain-named event-parameter variables — used by aw_ct Google Ads tag ──
  variables.push(epVar('Value',    'value'));     // reads ep.value (numeric conversion value)
  variables.push(epVar('Currency', 'currency'));  // reads ep.currency
  ...
  // Google Ads Enhanced Conversions — aw_ct tag per selected event
  // Fires on each event that maps to a Google Ads conversion action.
  // Enhanced Conversions path is enabled via userEmail/userPhone/userFirstName/userLastName.
  if (platList.includes('gads') && px.gads) {
    evList.forEach(key => {
      const gadsEv = GADS_EVENT[key];
      if (!gadsEv) return;
      const tid = trigMap[key];
      if (!tid) return;
      tags.push({
        name: 'ET - Google Ads EC - ' + key,
        type: 'aw_ct',
        tagId: nTagId(),
        parameter: [
          { type: 'TEMPLATE', key: 'conversionId',    value: '{{ET - Google Ads ID}}' },
          { type: 'TEMPLATE', key: 'conversionLabel', value: px.gads_label ? '{{ET - Google Ads Label}}' : '' },
          { type: 'TEMPLATE', key: 'conversionValue', value: '{{Value}}' },
          { type: 'TEMPLATE', key: 'currencyCode',    value: '{{Currency}}' },
          { type: 'BOOLEAN',  key: 'enableConversionLinker', value: 'true' },
          { type: 'TEMPLATE', key: 'userEmail',       value: '{{User Email}}' },
          { type: 'TEMPLATE', key: 'userPhone',       value: '{{User Phone}}' },
          { type: 'TEMPLATE', key: 'userFirstName',   value: '{{User First Name}}' },
          { type: 'TEMPLATE', key: 'userLastName',    value: '{{User Last Name}}' },
          { type: 'TEMPLATE', key: 'gclid',           value: '{{Click ID - gclid}}' },
        ],
        firingTriggerId: [tid],
        tagFiringOption: 'ONCE_PER_EVENT',
        notes: `EasyTrac — Google Ads Enhanced Conversions (${key}). conversionValue={{Value}}, currencyCode={{Currency}}.`,
      });
    });
  }
