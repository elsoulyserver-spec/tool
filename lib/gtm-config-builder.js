...
function buildServerConfig({
  ...
  pixelIds = {}, capiTokens = {},
  includeCapi = false,   // false → GA4-only (no CAPI tags/vars); true → full CAPI setup
} = {}) {
  _reset();
  ...
  // Pixel IDs + tokens — only when CAPI tags are included
  if (includeCapi) {
    if (px.meta)       variables.push(cVar('ET - Meta Pixel ID',       px.meta));
    if (px.tiktok)     variables.push(cVar('ET - TikTok Pixel ID',     px.tiktok));
    if (px.snap)       variables.push(cVar('ET - Snapchat Pixel ID',   px.snap));
  ...
  // ── CAPI tags — only when includeCapi = true ─────────────────────────────
  if (includeCapi) {
